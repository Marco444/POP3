...
enum pop3_states handle_write_stat(struct selector_key *key, pop3_current_command *current_command, struct commands_state *commands) {
    if(current_command->has_error){
        ...
        int i;
        for(i = 0; i < commands->inbox_data.total_size; i++){
            if(!commands->inbox_data.email_files->is_deleted){
                emails_total_size = commands->inbox_data.email_files->size;
                emails_count++;
            }
        }
        ...
        if(has_place){
            long offset = write_in_buffer(key, output);
            if(offset == -1){
                current_command->is_finished = true;
            }
        }
