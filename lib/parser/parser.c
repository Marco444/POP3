...
parser_feed(struct parser *p, const uint8_t c, struct commands_state * ctx, enum pop3_states pop_3_state, enum pop3_states * next_state, bool * read_command) {
    ...
    for(unsigned i = 0; i < n ; i++) {
        ...
        if(matched) {
            state[i].act1(&p->e1, c, ctx, pop_3_state, next_state, read_command);
            if(state[i].act2 != NULL) {
                p->e1.next = &p->e2;
                state[i].act2(&p->e2, c, ctx, pop_3_state, next_state, read_command);
            }
